...
describe('Checkout controller', function(){
	...
	describe("backToCart()", function(){
		it("takes back to /cart", function(){
			initController();
			$location.path = sinon.spy();

			$scope.backToCart();

			expect($location.path).toHaveBeenCalledWith("/cart");
		});
	});

	describe("backToFront()", function(){
		it("takes back to /", function(){
			initController();
			$location.path = sinon.spy();
