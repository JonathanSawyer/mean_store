describe("ProductService", function(){
	...
	describe("getAll", function(){
		...
		it("returns non-deleted products", function(){
			...
			prodSvc.getAll().then(function(res){
				data = res.data;
			});
			$httpBackend.flush();
		...
		it('gets deleted products also', function(){
			...
			prodSvc.getAll({deleted: true}).then(function(res){
				data = res.data;
			});
			$httpBackend.flush();
	...
	describe("getByCategory", function(){
		...
		it("returns products in a category", function(){
			...
			prodSvc.getByCategory(cat).then(function(res){
				data = res.data;
			});
			$httpBackend.flush();
	...
	describe("getById", function(){
		...
		it("returns a product by an id", function(){
			...
			$httpBackend.expectGET("/products/" + id)
				.respond(products[1]);

			var data;

			prodSvc.getById(id).then(function(res){
				data = res.data;
			});
			$httpBackend.flush();
