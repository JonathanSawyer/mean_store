	...
	describe('on init', function(){
		...
		beforeEach(function(){
			...
			orderSvc.getById = sinon.stub().returns({
				then : sinon.stub().callsArgWith(0, orderRes)
			});
			
			prodSvc.getById = sinon.stub();
			prodSvc.getById.withArgs('aa').returns({
				then : sinon.stub().callsArgWith(0, {data : products.aa})
			});
			prodSvc.getById.withArgs('bb').returns({
				then : sinon.stub().callsArgWith(0, {data : products.bb})
			});
		});
		...
		it('gets the order', function(){
			...
			expect(orderSvc.getById).toHaveBeenCalledWith(order._id);
