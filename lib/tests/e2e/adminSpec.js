...
describe('Admin main page', function(){
	...
	it("shows details of an order", function(){
		...
		this.page.orders.viewBtns.then(function(btns){
			that.page.orders.sentStatuses.map(function(row){
				return row.getText();
				...
				expect(orderDetails.receiver.firstName.getText()).toEqual(order.receiver.firstName);
				expect(orderDetails.receiver.lastName.getText()).toEqual(order.receiver.lastName);
				expect(orderDetails.receiver.address.getText()).toEqual(order.receiver.streetAddress);
				expect(orderDetails.receiver.postalCode.getText()).toEqual(order.receiver.postalCode);
				expect(orderDetails.receiver.city.getText()).toEqual(order.receiver.city);

				var total = order.products.reduce(function(prev, cur){
					return prev + (cur.quantity * cur.price);
				}, 0);

				expect(orderDetails.total.getText()).toContain(String(total));
				orderDetails.productNames.getText().then(function(names){
